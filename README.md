...
| 💬 라운드 로그 저장 | 각 라운드 힌트와 추측 내용 저장 -> 유저가 플레이어들의 멘트을 바탕으로 라이어 추측 |
...
좋아요! 😎  
`Streamlit Liar Game` 프로젝트에 사용된 **기술 스택**을 발표나 포트폴리오용 README에 넣기 적합하게 아래처럼 정리해드릴게요.
...
| **백엔드 모델**  | **EEVE-Korean-10.8B**                          | 한국어 특화 초거대 언어모델 (LLM), 라이어 게임 대화 생성 |
| **모델 실행**    | **Ollama**                                     | 로컬에서 LLM 모델 실행용 플랫폼 (Chat API 제공) |
---
